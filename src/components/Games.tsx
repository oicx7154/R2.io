...
export default function Games() {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://thumbnails.roblox.com/v1/places/gameicons?placeIds=${placeIds}&returnPolicy=PlaceHolder&size=512x512&format=Png&isCircular=false`
    )
      .then((res) => res.json())
      .then((data) => {
        const map: Record<number, string> = {};
        data.data?.forEach((item: { targetId: number; state: string; imageUrl: string }) => {
